  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Resumen de Flota</h1>
          <p className="text-dark-400">{today}</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={() => showToast('Filtros próximamente disponibles', 'info')}
            className="px-4 py-2 bg-dark-800 border border-dark-700 rounded-lg text-dark-300 hover:text-white hover:border-dark-600 transition-colors flex items-center gap-2"
            aria-label="Filtrar"
          >
            <span>Filtrar</span>
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div 
          ...
          className="bg-gradient-to-br from-dark-900 to-dark-950 border border-green-500/20 rounded-xl p-6 shadow-lg shadow-green-500/5 hover:shadow-green-500/10 transition-all cursor-pointer"
          role="button"
          tabIndex={0}
          onKeyDown={(e) => {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-dark-300 text-xs font-semibold uppercase tracking-wider mb-2">OPERATIVOS</p>
              <p className="text-4xl font-bold text-white mb-1">{operativos}</p>
            </div>
          ...
          <p className="text-dark-400 text-sm font-medium">Vehículos al día</p>
        </div>

        {/* Atención */}
        <div 
          onClick={() => navigate('/mantenimientos?tab=proximos')}
          className="bg-gradient-to-br from-dark-900 to-dark-950 border border-orange-500/20 rounded-xl p-6 shadow-lg shadow-orange-500/5 hover:shadow-orange-500/10 transition-all cursor-pointer"
          role="button"
          tabIndex={0}
          onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-dark-300 text-xs font-semibold uppercase tracking-wider mb-2">ATENCIÓN</p>
              <p className="text-4xl font-bold text-white mb-1">{proximos}</p>
            </div>
          ...
          <p className="text-dark-400 text-sm font-medium">Próximos mantenimientos</p>
        </div>

        {/* Crítico */}
        <div 
          onClick={() => navigate('/mantenimientos?tab=vencidos')}
          className="bg-gradient-to-br from-dark-900 to-dark-950 border border-red-500/20 rounded-xl p-6 shadow-lg shadow-red-500/5 hover:shadow-red-500/10 transition-all cursor-pointer"
          role="button"
          tabIndex={0}
          onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-dark-300 text-xs font-semibold uppercase tracking-wider mb-2">CRÍTICO</p>
              <p className="text-4xl font-bold text-white mb-1">{vencidos}</p>
            </div>
          ...
          <p className="text-dark-400 text-sm font-medium">Mantenimientos vencidos</p>
        </div>

        {/* Finanzas */}
        <div className="bg-gradient-to-br from-dark-900 to-dark-950 border border-blue-500/20 rounded-xl p-6 shadow-lg shadow-blue-500/5 hover:shadow-blue-500/10 transition-all">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-dark-300 text-xs font-semibold uppercase tracking-wider mb-2">FINANZAS</p>
              <p className="text-4xl font-bold text-white mb-1">{formatCurrency(costoMes)}</p>
            </div>
          ...
          <p className="text-dark-400 text-sm font-medium">Coste este mes</p>
        </div>
      </div>

      {/* Quick Alerts */}
      <div className="bg-gradient-to-br from-dark-900 to-dark-950 border border-dark-800 rounded-xl p-6 shadow-lg">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-white">Vista rápida de alertas</h2>
          <button 
            onClick={handleVerTodo}
            className="text-primary-400 hover:text-primary-300 text-sm flex items-center gap-1 font-medium"
        ...
        <div className="space-y-3">
          {alerts.length === 0 ? (
            <div className="text-center py-8 text-dark-400">
              ...
              <p className="text-xs text-dark-500 mt-1">Todos los mantenimientos están al día</p>
            </div>
          ) : (
            alerts.map((alert: any) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center gap-4 p-4 bg-dark-800/50 border border-dark-700/50 rounded-lg hover:bg-dark-800 hover:border-dark-600 transition-all cursor-pointer shadow-sm"
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-dark-300 text-sm">{alert.details}</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-dark-400 hover:text-primary-400 transition-colors" />
                </div>
              )
            })
          )}
