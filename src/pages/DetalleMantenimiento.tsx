  ...
  if (!mantenimiento) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        <Wrench className="w-16 h-16 text-dark-500 mb-4" />
        <h2 className="text-xl font-semibold text-white mb-2">Mantenimiento no encontrado</h2>
        <p className="text-dark-400 mb-4">El mantenimiento que buscas no existe o ha sido eliminado.</p>
        <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-white">Detalle del Mantenimiento</h1>
          </div>
        </div>
        <button
          onClick={() => navigate('/mantenimientos')}
      ...
      <div className="bg-dark-900 border border-dark-800 rounded-lg p-6">
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white mb-1">{mantenimiento.tipo}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-dark-800 rounded-lg p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <label className="text-sm font-medium text-dark-300">Vehículo</label>
            </div>
            <p className="text-white font-semibold text-lg">{vehiculo?.modelo || 'N/A'}</p>
            <p className="text-dark-400 text-sm mt-1">Matrícula: {vehiculo?.matricula || 'N/A'}</p>
            {vehiculo && (
              <button
          ...
          {mantenimiento.costo !== undefined && (
            <div className="bg-dark-800 rounded-lg p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <label className="text-sm font-medium text-dark-300">Coste</label>
              </div>
              <p className="text-white font-semibold text-2xl">{formatCurrency(mantenimiento.costo)}</p>
            </div>
          )}
          ...
          {mantenimiento.fechaVencimiento && (
            <div className="bg-dark-800 rounded-lg p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <label className="text-sm font-medium text-dark-300">Fecha Objetivo</label>
              </div>
          ...
          {mantenimiento.odometro !== undefined && (
            <div className="bg-dark-800 rounded-lg p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <label className="text-sm font-medium text-dark-300">Km Objetivo</label>
              </div>
              <p className="text-white font-semibold text-2xl">{formatNumber(mantenimiento.odometro)}</p>
              <p className="text-dark-400 text-sm mt-1">kilómetros</p>
          ...
          <div className="bg-dark-800 rounded-lg p-4 md:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3 mb-3">
              <Calendar className="w-5 h-5 text-purple-400" />
              <label className="text-sm font-medium text-dark-300">Vencimiento</label>
            </div>
            <p className="text-white font-medium">{mantenimiento.vencimiento}</p>
          </div>
        </div>
        ...
        {mantenimiento.notas && (
          <div className="mt-6 pt-6 border-t border-dark-800">
            <label className="block text-sm font-medium text-dark-300 mb-3">Notas</label>
            <div className="bg-dark-800 rounded-lg p-4">
              <p className="text-white whitespace-pre-wrap">{mantenimiento.notas}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {vehiculo && (
          <div className="bg-dark-900 border border-dark-800 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-1">Modelo</label>
                <p className="text-white">{vehiculo.modelo}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-1">Tipo</label>
                <p className="text-white">{vehiculo.tipo}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-1">Año</label>
                <p className="text-white">{vehiculo.año}</p>
              </div>
              {vehiculo.kilometrajeActual !== undefined && (
                <div>
                  <label className="block text-sm font-medium text-dark-300 mb-1">Kilometraje Actual</label>
                  <p className="text-white">{formatNumber(vehiculo.kilometrajeActual)} km</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-dark-900 border border-dark-800 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-sm font-medium text-dark-300 mb-1">Estado</label>
            ...
            <div>
              ...
              <p className="text-white">{mantenimiento.tipo}</p>
            </div>
            {mantenimiento.fechaVencimiento && (
              <div>
                <label className="block text-sm font-medium text-dark-300 mb-1">Fecha Programada</label>
