  ...
  const dropdown = isOpen && dropdownRect
    ? createPortal(
        <div
          ...
          className="fixed z-[60] bg-dark-800 border border-dark-700 rounded-lg shadow-lg max-h-60 overflow-auto w-full"
          style={{
        ...
        >
          <div className="p-2 border-b border-dark-700 bg-dark-900">
            <input
              ...
              placeholder="Buscar..."
              className="w-full bg-dark-900 border border-dark-700 rounded-md px-3 py-2 text-sm text-white placeholder:text-dark-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
            />
          </div>
          <div className="py-1">
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm transition-colors flex items-center gap-2 ${
                    value === option.value
                      ...
                      : 'text-white'
                  } ${highlightedIndex === index ? 'bg-dark-700' : ''}`}
  ...
  return (
    <div className={`relative ${className}`} ref={selectRef}>
      <button
        ...
        className={`w-full bg-dark-800 ${triggerBorderClasses} rounded-lg text-white focus:outline-none focus:ring-2 flex items-center justify-between ${baseHeightClasses} ${triggerPaddingClasses} disabled:opacity-60 disabled:cursor-not-allowed`}
      ...
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {leadingIcon && (
            <span className="text-dark-400 flex-shrink-0">{leadingIcon}</span>
          )}
          <span
        ...
        <ChevronDown
          className={`w-4 h-4 text-dark-400 flex-shrink-0 ml-2 transition-transform ${
            isOpen ? 'rotate-180' : ''
          }`}
        />
      </button>
      ...
      {helperText && !hasError && (
        <p id={helperId} className="mt-1 text-xs text-dark-400">
          {helperText}
        </p>
      )}
